...
model = RandomForestClassifier(n_estimators=50, max_depth=2, min_samples_leaf=2, random_state=seed)  # Número de árvores na floresta (ajuste conforme necessário)
...
# Criar um DataFrame com as duas características e o rótulo
wine_df = pd.DataFrame(data=x_normalized[:, [feature1_index, feature2_index]], columns=[wine.feature_names[feature1_index], wine.feature_names[feature2_index]])
wine_df['target'] = y
...
x_train, x_test, y_train, y_test = train_test_split(wine_df[[wine.feature_names[feature1_index], wine.feature_names[feature2_index]]], y, test_size=0.2, random_state=seed)
...
plot_decision_regions(x_test.to_numpy(), y_test, clf=model)
plt.xlabel(wine.feature_names[feature1_index])
plt.ylabel(wine.feature_names[feature2_index])
